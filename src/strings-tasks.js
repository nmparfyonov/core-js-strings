...
function removeLastOccurrences(str, value) {
  ...
  const last = str.substring(index + value.length);
...
function sumOfCodes(str) {
  return str ? str.split('').reduce((a, b) => a + +b.charCodeAt(0), 0) : 0;
}
...
function isPalindrome(str) {
  const lowercase = str.toLowerCase().replace(/\W/gi, '');
...
function findLongestWord(sentence) {
  ...
  const arrNum = arr.map((a) => a.length);
  const max = Math.max(...arrNum);
  const index = arrNum.indexOf(max);
  return arr[index];
}
